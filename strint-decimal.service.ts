...
export class StrintDecimalService {
  ...
  add(x, y) {
    ...
    var response = this.convertDigitsDecimal(x, y);
    x = response.x;
    y = response.y;
    var result = "";
    ...
    if (this.isPositive(x) && this.isPositive(y)) {
      result = this.addPositive(x, y);
      if (response.maxDecimalCount > 0)
          return this.splitValue(result, result.length - response.maxDecimalCount)

      return result;
    } else if (this.isNegative(x) && this.isNegative(y)) {
      result = this.negate(this.addPositive(this.abs(x), this.abs(y)));
      if (response.maxDecimalCount > 0)
          return this.splitValue(result, result.length - response.maxDecimalCount)
  ...
  mul(lhs, rhs) {
    ...
    var response = this.convertDigitsDecimal(lhs, rhs);
    lhs = response.x;
    rhs = response.y;
    ...
    var result = (this.sameSign(lhs, rhs) ? absResult : this.negate(absResult));
    if (response.maxDecimalCount > 0)
        return this.splitValue(result, result.length - (response.maxDecimalCount * 2))
  ...
  div(dividend, divisor) {
    ...
    var response = this.convertDigitsDecimal(dividend, divisor);
    dividend = response.x;
    divisor = response.y;
    ...
    var result = (this.sameSign(dividend, divisor) ? absResult : this.negate(absResult));
